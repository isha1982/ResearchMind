...
if st.session_state.running and not st.session_state.done:
    ...
    try:
        ...
        if "503" in error_text or "unavailable" in error_text or "high demand" in error_text:
            st.error("🤖 AI service is temporarily busy. Please wait a few seconds and try again.")
        elif "429" in error_text or "quota" in error_text or "rate limit" in error_text:
            st.error("⚠️ Gemini API free limit has been reached. Please wait and try again later.")
        elif "timeout" in error_text or "timed out" in error_text:
            st.error("🌐 The request took too long. Please check your internet connection and try again.")
        elif "not_found" in error_text or ("model" in error_text and "not available" in error_text):
            st.error("⚙️ The selected AI model is currently unavailable. Please update the model and try again.")
        else:
            st.error("⚠️ Something went wrong while generating the report. Please try again.")
...
<div class="watermark">Made by Isha</div>
""", unsafe_allow_html=True)